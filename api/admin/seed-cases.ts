...
import { FREE_CASE_ID, FREE_CASE, FREE_SOLUTION, SHOWCASE_CASE_ID, SHOWCASE_CASE, SHOWCASE_SOLUTION, SECOND_CASE_ID, SECOND_CASE, SECOND_SOLUTION } from "../_lib/seedData.js";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        await db.collection('case_solutions').doc(SECOND_CASE_ID).set(SECOND_SOLUTION);

        return res.status(200).json({ cleaned, seeded: [FREE_CASE_ID, SHOWCASE_CASE_ID, SECOND_CASE_ID] });
    } catch (error) {
